...
unit_to_ppb = {'ppt':1e-3, 'ng/l':1e-3, 'ppb':1, 'ug/l':1, 'ppm':1000, 'mg/l':1000}
float_match = '[-]?\d+[\.]?\d*(e[-]?\d+)?'     #matches floats like '-3.5e4' or '7' or '245.13' or '1e-15'


def get_calibration_function(ICPMS_data, mass, unit='ug/l', units=[], ax=None):
    '''
    ...
    calibration_keys = [key for key in ICPMS_data.keys() if len([u for u in units if u in key])>0]
    ...
    if ax is not None:
        ...
        ax.plot(counts_to_amount(cal_counts), cal_counts, 'r--')
